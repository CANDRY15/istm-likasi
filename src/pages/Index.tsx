...
import { Search, BookOpen, GraduationCap, FileText, ArrowRight, Award, Globe, Clock } from "lucide-react";
import { useState } from "react";
import { useTFCs, useLibraryDocs, useArticles } from "@/hooks/useContent";

const Index = () => {
  const [searchQuery, setSearchQuery] = useState("");
  const { data: tfcs = [] } = useTFCs();
  const { data: libraryDocs = [] } = useLibraryDocs();
  const { data: articles = [] } = useArticles();

  const stats = [
    { icon: BookOpen, value: libraryDocs.length.toString(), label: "Documents" },
    { icon: GraduationCap, value: tfcs.length.toString(), label: "TFC Publiés" },
    { icon: FileText, value: articles.length.toString(), label: "Articles Revue" },
  ];
  ...
  const features = [
    ...
    },
    {
      ...
      description: "Consultez les TFC des étudiants de tous les départements.",
      link: "/tfc",
            ...
            <div className="max-w-4xl mx-auto text-center">
              ...
              <div className="max-w-2xl mx-auto mb-10 animate-slide-up" style={{ animationDelay: "0.2s" }}>
                <div className="relative">
                  ...
                  />
                  <Button variant="gold" size="lg" className="absolute right-2 top-1/2 -translate-y-1/2">
                    Rechercher
                  </Button>
                </div>
              </div>
